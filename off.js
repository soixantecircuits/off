...
OffTracker = function(options){
  ...
  this.bananas = document.querySelectorAll('[data-off]');
  this.trackName = options.trackName || 'track';
  this.trackArray = store.get(this.trackName) || [],
  this.init();
  return this;
}
OffTracker.prototype = {
  init: function(){
    ...
    for(var z =0; z < _self.bananas.length; z++){
      ...
      elem.addEventListener("click", recordEvent.bind(elem), false);
      elem.addEventListener("touchend", recordEvent.bind(elem), false);
    }
  },
