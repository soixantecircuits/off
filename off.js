...
function JSON2CSV(objArray, label, doubleQuote) {
    label = label || true;
...
var bananas = document.querySelectorAll('[data-off]'),
trackName = 'track',
trackArray = store.get(trackName) || [],
init = function(){
  for(var z =0; z < bananas.length; z++){
    var elem = bananas[z],
    recordEvent = function(){
        ...
        trackArray.push(eventTracked);
        store.set(trackName, trackArray);
        return false;
    };
...
clearTrack = function(){
  store.remove(trackName);
},
exportData = function(){
  var csv = JSON2CSV(trackArray);
